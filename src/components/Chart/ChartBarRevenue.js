...
export default function ChartBarRevenue(props) {
  // Lấy năm hiện tại
  ...
  const getChartData = async () => {
    try {
      ...
      const result = await axios.get(
        `https://localhost:7208/api/Statistics/GetTotalMonthYear/TotalMonthYear?year=${inputYear}`
      );
      if (result.data && result.data.monthRevenue) {
  ...
  return (
    <>
      ...
      {chartData && chartData.monthRevenue && chartData.monthRevenue.length > 0 ? (
        <Bar
          data={{
            labels: chartData.monthRevenue.map((item) => {
            ...
            datasets: [
              {
                label: `Doanh thu theo từng tháng - ${chartData.monthRevenue[0].year}`,
                backgroundColor: 'rgba(75,192,192,0.4)',
                borderColor: 'rgba(75,192,192,1)',
                borderWidth: 1,
                hoverBackgroundColor: 'rgba(75,192,192,0.6)',
                hoverBorderColor: 'rgba(75,192,192,1)',
                data: chartData.monthRevenue.map((item) => item.revenue)
              }
            ]
          }}
          options={{
